...
import random
from utils import SFeature, SBundle
...
class Chain: # type and syntactic feature configuration
    def __init__(self, type, features):
        ...
        self.features = SBundle(features)            
    def __repr__(self):
        ...
        for first_feat, exps in feat_to_exp.items():
            ...
            if is_sel(first_feat): # apply Merge
                for exp in exps: # iterate over exps
                    try: matches = feat_to_exp[SFeature(cat, name=first_feat.name, mfeats=first_feat.mfeats)]
                ...
                for exp in exps: # iterate over exps
                    move_indices = [ind for ind in range(1, len(exp)) if is_lee(exp[ind].features[0]) and exp[ind].features[0].name == first_feat.name]
...
def mg2mcfg(mg, start_exp, useful=False):
    feat_to_exp = {}
    mcfg_dict = {}
                ...
                for item in line.split(";"): # for each lexical item
                    if item:
                        ...
                        for piece in pieces: # parse each feature into type and name
                            ...
                            features.append(SFeature(type, name))
                        mg[item_mor] = SBundle(features)
    return mg, start
