...
def mg_to_plot(mg, n, t="", fresh=set()): # grammar, path sans extension, title, fresh categories
    ftsize = '50'
    graph = pd.Dot(graph_type='digraph', label=t, fontsize=ftsize, rankdir='RL', layout='dot', esep=5)
    graph.add_node(pd.Node(head_name, style='filled', fillcolor='lightgray',  fontsize=ftsize)) # fillcolor='lightskyblue'
    counter = 0
    ...
    for (b, phons) in edges.items():
        ...
        node_cat = pd.Node(li_cat, fontsize=ftsize)
        if li_first != None:
            node_first = pd.Node(li_first, fontsize=ftsize)
        else:
            node_first = pd.Node("z{}".format(counter), label="", style='dashed', fontsize=ftsize)
            counter += 1
        ...
        label="{}\n{}".format(pretty_label([pname(x) for x in phons]), pf(b))
        ...
        graph.add_edge(pd.Edge(node_first, node_cat, label=label,  fontsize=ftsize, color=edge_color, penwidth=edge_width))
